...
@php
    // =========================
    // NEW: Safe client id untuk kasus transaksi tanpa id_klien
    // - ambil dari $client kalau ada
    // - fallback ke parameter route id_account kalau ada
    // =========================
    $clientIdSafe = null;
    if (isset($client) && $client && isset($client->id_account) && $client->id_account) {
        $clientIdSafe = $client->id_account;
    }
    if (!$clientIdSafe) {
        $clientIdSafe = request()->route('id_account');
    }

    // kalau route pakai dummy "0", anggap tidak ada client
    if ($clientIdSafe === '0' || $clientIdSafe === 0) {
        $clientIdSafe = null;
    }

    // =========================
    // NEW: id_account untuk kebutuhan route yang PARAMETER-NYA WAJIB
    // (kalau route kamu /{id_listing}/{id_account} wajib, isi dummy "0")
    // =========================
    $clientIdForRoute = $clientIdSafe ?: '0';

    // =========================
    // NEW: data client aman untuk dipakai di JS
    // =========================
    $clientNameSafe  = (isset($client) && $client && isset($client->nama)) ? $client->nama : null;
    $clientPhoneSafe = (isset($client) && $client && isset($client->nomor_telepon)) ? $client->nomor_telepon : null;

    // =========================
    // NEW: transaksi id (kalau ada) biar update status ke transaction by id_transaction
    // - kalau controller kirim $transaction atau $transactionId, akan kepakai
    // =========================
    $transactionIdSafe = null;
    if (isset($transaction) && $transaction && isset($transaction->id_transaction) && $transaction->id_transaction) {
        $transactionIdSafe = $transaction->id_transaction;
    }
    if (!$transactionIdSafe && isset($transactionId) && $transactionId) {
        $transactionIdSafe = $transactionId;
    }
@endphp
        ...
        <div class="col-md-4">
            ...
            <div class="card shadow-sm rounded">
                ...
                <div class="card-body">
                    ...
                    {{-- =========================
                        ORIGINAL (disimpan, supaya tidak “hilang”):
                        <form action="{{ route('dashboard.updateOwner', ['id_listing' => $property->id_listing, 'id_account' => $client->id_account]) }}" method="POST">
                    ========================= --}}
                    {{-- <form action="{{ route('dashboard.updateOwner', ['id_listing' => $property->id_listing, 'id_account' => $client->id_account]) }}" method="POST"> --}}

                    {{-- =========================
                        NEW: route WAJIB id_account -> selalu kirim $clientIdForRoute (dummy "0" kalau kosong)
                    ========================= --}}
                    <form action="{{ route('dashboard.updateOwner', ['id_listing' => $property->id_listing, 'id_account' => $clientIdForRoute]) }}" method="POST">
                        @csrf

                        {{-- NEW: payload aman untuk controller update --}}
                        <input type="hidden" name="id_listing" value="{{ $property->id_listing }}">
                        <input type="hidden" name="id_account" value="{{ $clientIdForRoute }}">
                        <input type="hidden" name="id_transaction" value="{{ $transactionIdSafe ?: '' }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select id="status-select" name="status" class="form-select">
                                ...
                                {{-- =========================
                                    NEW: Dropdown status jadi 1 semua (tanpa disable)
                                ========================= --}}
                        ...
                        <!-- Notes (selalu ada biar catatan bisa dikirim kapan pun) -->
                        <div class="mb-3">
                            <label for="message-box" class="form-label fw-semibold">Catatan Admin:</label>
                            <textarea id="message-box" name="comment" class="form-control" rows="4" placeholder="(Opsional) Tulis catatan...">Perkiraan __ hari lagi selesai</textarea>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // =========================
    // NEW: client safe untuk JS (biar tidak error kalau $client null)
    // =========================
    const clientName  = @json($clientNameSafe);
    const clientPhone = @json($clientPhoneSafe);
    const lokasiProp  = @json($property->lokasi);
    ...
    function renderDynamic(selected) {
            ...
            if (!clientPhone || !clientName) {
                dynamicSection.innerHTML = `
                    <div class="alert alert-warning" role="alert">
                        Data client belum ada (id_klien kosong), jadi tombol WhatsApp tidak bisa dibuat.
                    </div>
                `;
            } else {
                const phone = String(clientPhone).replace(/^0+/, '');
                const text  = encodeURIComponent(`Halo ${clientName}, saya ingin memastikan apakah ada informasi yang bisa saya bantu terkait rumah di ${lokasiProp}?`);
                const url   = `https://wa.me/62${phone}?text=${text}`;

                dynamicSection.innerHTML = `
                    <div class="mb-3">
                        <a href="${url}" target="_blank" class="btn btn-sm btn-success w-100">
        ...
        if (selected === 'BuyerMeeting') {
            if (!clientPhone || !clientName) {
                dynamicSection.innerHTML = `
                    <div class="alert alert-warning" role="alert">
                        Data client belum ada (id_klien kosong), jadi tombol WhatsApp Buyer Meeting tidak bisa dibuat.
                    </div>
                `;
            } else {
                const phone = String(clientPhone).replace(/^0+/, '');

                const msg = `📅 Reminder Buyer Meeting
Obyek: ${lokasiProp}
Hari: Senin
Tanggal: {{ date('Y-m-d') }}
Pukul: 13:00
...
🌐 GMAP: https://maps.app.goo.gl/6gR4s3xDtEaeEya26?g_st=awb`;
                ...
                const url = `https://wa.me/62${phone}?text=${encodeURIComponent(msg)}`;
    ...
    // ✅ tampilkan dynamic sesuai status awal (kalau status awal FollowUp/BuyerMeeting/Closing)
    renderDynamic(statusSelect.value);
});
</script>
