    ...
    public function home()
{
    ...
    if (!$idaccount) {
        // ❌ Tidak login
        $hotListings = \App\Models\Property::query()
            ->leftJoin('agent', 'agent.id_agent', '=', 'property.id_agent')
            ->where('property.status', 'Tersedia')
            ->whereNotNull('property.gambar')
            ->where('property.gambar', '!=', '')
        ...
        if (!$kecamatan && !$kota && !$provinsi) {
            ...
            // ===== base query untuk semua langkah (sudah join agent) =====
            ...
            if ($kecamatan && $kota) {
                $results = (clone $query)
                    ->where('property.kecamatan', $kecamatan)
                    ->where('property.kota', $kota)
                    ->take(6)
                    ->get();
            }
            ...
            if ($results->count() < 6 && $kota) {
                ...
                $more = (clone $query)
                    ->where('property.kota', $kota)
                    ->when($kecamatan, fn($q) => $q->where('property.kecamatan', '!=', $kecamatan))
                    ->take($remaining)
                    ->get();
            ...
            if ($results->count() < 6 && $provinsi) {
                $remaining = 6 - $results->count();
                $more = (clone $query)
                    ->where('property.provinsi', $provinsi)
                    ->when($kota, fn($q) => $q->where('property.kota', '!=', $kota))
                    ->take($remaining)
                    ->get();
                $results = $results->concat($more);
            }
            ...
            if ($results->count() < 6) {
                $remaining = 6 - $results->count();
                $more = \App\Models\Property::query()
                    ->leftJoin('agent', 'agent.id_agent', '=', 'property.id_agent')
                    ->where('property.status', 'Tersedia')
                    ->whereNotNull('property.gambar')
                    ->where('property.gambar', '!=', '')
